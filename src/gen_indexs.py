...
# PC writes barcode ¡æ D6980-D6999(L1)/D7980-D7999(L2)
...
al("BARCODE COPY L1")
# New barcode from PC (D6980-D6999) ¡æ copy to HMI display (D7220-D7239)
a("LD>","D6980"); ac("K0")
...
al("L1 MODEL LOOKUP")
# Barcode mode: D7001 = validated barcode target ¡æ find matching model row
for k in range(1, 26):
    base = 300 + (k-1) * 9
    a("LD","M520"); a("LDD>=","D7000"); ac("K1"); a("ANDD=","D7001"); ac(f"D{base+1}")
    a("MOV",f"K{k}"); ac("D0")
# Both modes: D0==model# ¡æ load target + apply correction
for k in range(1, 26):
    base = 300 + (k-1) * 9
...
al("BARCODE CLEAR")
# Cycle complete/NG ¡æ clear PC barcode buffer (barcode mode only)
a("LD","M107"); a("OR","M123")
a("OR","M108"); a("OR","M109")
a("OR","M124"); a("OR","M125")
a("OR","M300")
...
a("END","")
wr("C:\\WorkSpace\\REF\\src\\indexs.csv")
