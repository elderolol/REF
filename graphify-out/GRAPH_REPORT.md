# Graph Report - F:/WorkSpace/REF/src_md  (2026-06-03)

## Corpus Check
- 28 files · ~36,773 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3713 nodes · 6412 edges · 463 communities (452 shown, 11 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 165 edges (avg confidence: 0.88)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- [[_COMMUNITY_HMI & System Configuration|HMI & System Configuration]]
- [[_COMMUNITY_Device Map & Step Machine|Device Map & Step Machine]]
- [[_COMMUNITY_Agent & OpenCode|Agent & OpenCode]]
- [[_COMMUNITY_Operation Screen & Gun Selection|Operation Screen & Gun Selection]]
- [[_COMMUNITY_System Functions & Modes|System Functions & Modes]]
- [[_COMMUNITY_CSV Documentation (ad)|CSV Documentation (ad)]]
- [[_COMMUNITY_HMI Screens & Navigation|HMI Screens & Navigation]]
- [[_COMMUNITY_JSON Metadata Structure|JSON Metadata Structure]]
- [[_COMMUNITY_Alarm Per-Line Plan|Alarm Per-Line Plan]]
- [[_COMMUNITY_Barcode & Model Lookup|Barcode & Model Lookup]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 400|Community 400]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]

## God Nodes (most connected - your core abstractions)
1. `REFRIGER CHARGING MACHINE — Function Spec: ad.csv` - 90 edges
2. `device_map` - 83 edges
3. `device_map` - 83 edges
4. `device_map` - 83 edges
5. `device_map` - 83 edges
6. `read_steps` - 82 edges
7. `set_steps` - 82 edges
8. `rst_steps` - 82 edges
9. `out_steps` - 82 edges
10. `read_steps` - 82 edges

## Surprising Connections (you probably didn't know these)
- `방폭 Explosion-Proof Type` --semantically_similar_to--> `§7 Explosion-Proof Configuration`  [INFERRED] [semantically similar]
  .opencode/memory/project.md → Notes/REFRIGER_CHARGING_MACHINE.md
- `비방폭 Non-Explosion-Proof Type` --semantically_similar_to--> `Non-Explosion-Proof Configuration (비방폭)`  [INFERRED] [semantically similar]
  .opencode/memory/project.md → Notes/REFRIGER_CHARGING_MACHINE.md
- `Door Limit Sensor (좌/우, 방폭 전용)` --semantically_similar_to--> `Door Limit Sensor Subsystem (방폭 전용)`  [INFERRED] [semantically similar]
  .opencode/memory/project.md → Notes/REFRIGER_CHARGING_MACHINE.md
- `Door Open Alarm` --semantically_similar_to--> `Door Open Alarm (M-relay Latch)`  [INFERRED] [semantically similar]
  .opencode/memory/project.md → Notes/REFRIGER_CHARGING_MACHINE.md
- `Alarm-Only Design — No Interlock, No Emergency Stop` --semantically_similar_to--> `Design Choice: Alarm Without Emergency Stop or Interlock`  [INFERRED] [semantically similar]
  .opencode/memory/project.md → Notes/REFRIGER_CHARGING_MACHINE.md

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Vacuum Control Subsystem** — src_md_gunvac, src_md_unitvac, src_md_vacchec, src_md_vacuum_measurement_data, src_md_alarm_latch_relays [INFERRED 0.95]
- **Sensor Data Pipeline** — src_md_485, src_md_ad, src_md_setting, src_md_gmes, src_md_sensor_data_registers [INFERRED 0.85]
- **Two-Line L0/L1 Architecture** — src_md_step_sequence_l0, src_md_step_sequence_l1, src_md_two_line_architecture, src_md_gunvac, src_md_unitvac, src_md_vacchec, src_md_refinj, src_md_ad, src_md_spc [INFERRED 0.95]
- **Vacuum Process Sequence (GunVac → UnitVac → VacCheck)** — notes_csv_func_gunvac_gunvaccsv, notes_csv_func_unitvac_unitvaccsv, notes_csv_func_vacchec_vaccheccsv [EXTRACTED 1.00]
- **Injection Process Type Branching** — notes_csv_func_refinj_refinjcsv, notes_csv_func_refinj_injectiontypes, notes_csv_func_refinj_volumecalculation, notes_csv_func_refinj_tolerancecheck [EXTRACTED 1.00]
- **Alarm Source Modules** — notes_csv_func_alarm_alarmcsv, notes_csv_func_gunvac_gunvaccsv, notes_csv_func_unitvac_unitvaccsv, notes_csv_func_vacchec_vaccheccsv, notes_csv_func_refinj_refinjcsv, notes_csv_func_setting_settingcsv, notes_csv_func_idata_idatacsv [EXTRACTED 1.00]
- **Graphify Agent Workflow** — agents_md_graphify_system, agents_md_graphify_commands, agents_md_graphify_update_hook [EXTRACTED 1.00]

## Communities (463 total, 11 thin omitted)

### Community 0 - "HMI & System Configuration"
Cohesion: 0.05
Nodes (74): REFRIGER CHARGING MACHINE - 2LINE4GUN Configuration, 알람 화면 (Alarm Screen) Button, 부저 정지 (Buzzer Stop) Button, 주입건 A (Injection Gun A), 주입건 C (Injection Gun C), 주입건 D (Injection Gun D), 주입량 설정화면 (Injection Amount Setting Screen) Button, 인터락 미사용 (Interlock Not Used) Button (+66 more)

### Community 1 - "Device Map & Step Machine"
Cohesion: 0.05
Nodes (74): out_steps, read_steps, rst_steps, set_steps, type, out_steps, read_steps, rst_steps (+66 more)

### Community 2 - "Agent & OpenCode"
Cohesion: 0.06
Nodes (51): agent, build, explore, general, plan, model, command, type (+43 more)

### Community 3 - "Operation Screen & Gun Selection"
Cohesion: 0.06
Nodes (44): 2LINE2GUN_OIL Operation Screen (Unit 2), 자동스탭 (Auto Step), 차징시간 (Charging Time), 컨베어 인터록 사용 (Conveyor Interlock Use), R32건 (R32 Gun), R410A건 (R410A Gun), 건 선택 (Gun Selection), 모델 선택 (Model Selection) (+36 more)

### Community 4 - "System Functions & Modes"
Cohesion: 0.07
Nodes (40): REFRIGER CHARGING MACHINE, ALARM_RESET, BARCODE_USE, BUZZER_STOP, GUN_EXHAUST, INTERLOCK_NOT_USED, MANUAL_MODE, PRODUCT_EXHAUST (+32 more)

### Community 5 - "CSV Documentation (ad)"
Cohesion: 0.07
Nodes (37): 1. Purpose, 2. Inputs, 3. Outputs, 5. Filtering, 7. IL Mnemonics, code:block5 (Filtered = α × CurrentRaw + (1 - α) × PreviousFiltered), REFRIGER CHARGING MACHINE — Function Spec: ad.csv, 10. IL Mnemonics (+29 more)

### Community 6 - "HMI Screens & Navigation"
Cohesion: 0.09
Nodes (35): LS IXP2-1200 HMI, Mitsubishi PLC, Operation Screen, Parameter Setting Screen, User Setting Screen, Alarm Screen, Screen Navigation Flow, Momentary Button Rule (+27 more)

### Community 7 - "JSON Metadata Structure"
Cohesion: 0.08
Nodes (29): blocks, meta, block_count, cpu, generated, source_file, total_steps, test_cases (+21 more)

### Community 8 - "Alarm Per-Line Plan"
Cohesion: 0.08
Nodes (31): Alarm Page Navigation by D Register, Alarm Per-Line Separation Plan, ALARM RESET (per-line), Alarm Per-Line Separation Plan, Bombe — configurable (depends on D330), Buzzer (M4C → Y30) — single, shared, code:block1 (L40 OR L41 OR L42 OR L43 OR L44 OR L45 OR L46 OR L47), Current (direct execute) (+23 more)

### Community 9 - "Barcode & Model Lookup"
Cohesion: 0.07
Nodes (30): 6-1. Barcode = Not Used (수동 모드), 6-2. Barcode = Used (PC 연동 모드), 6. Barcode Model Lookup Logic, code:block1 (사용자가 MODEL 버튼(M40B)을 누름 → HMI 숫자 입력창 표시), code:block2 (PC가 Ethernet으로 D180/D230 (Barcode Write Area)에 직접 기록), 1-1. Gun Type (솔레노이드 구조), 1-2. Oil Mode (주입 모드), 1-3. 방폭 Type (Enclosure) (+22 more)

### Community 10 - "Community 10"
Cohesion: 0.10
Nodes (30): out_steps, read_steps, rst_steps, set_steps, type, out_steps, read_steps, rst_steps (+22 more)

### Community 11 - "Community 11"
Cohesion: 0.08
Nodes (28): Alarm Detection and Handling System, Analog Input Processing (AI Raw → EU Scaling), Barcode PC↔PLC Data Flow (Write Area → Working Area → Clear), System Configuration Registers (D330-D339), D (Data Register) Device Map (Even-Address Rule), Line Independence Principle, Per-Line Parameter Register Layout (D0-D29 L0, D30-D59 L1), Line/Gun System Configuration (Supports 1L/1G ~ 2L/4G) (+20 more)

### Community 12 - "Community 12"
Cohesion: 0.08
Nodes (26): 6. 센서 Type 선택 아키텍처 (미래 확장), code:block6 ([Dxxx Sensor Type Select]), 4-1. System Configuration (D270~D276), 4-2. Parameter Settings — Line 0 (D0~D29), 4-3. Parameter Settings — Line 1 (D30~D59), 4. Parameters 관리, 3-10. SPC 누계 — Line 1 (D290~D299), 3-11. HSC Parameters (D310~D31F) (+18 more)

### Community 13 - "Community 13"
Cohesion: 0.10
Nodes (20): Additional Device Types (not in MAIN.csv), code:block1 (REF/), code:block2 (FF FE  4D 00 43 00 5F 00 …      ← BOM, then "MC_..." in UTF-), Development Rules, Device Prefix Frequency (MAIN.csv), Directory Structure, External Reference, File Format (GX Works2 IL CSV) (+12 more)

### Community 14 - "Community 14"
Cohesion: 0.10
Nodes (19): L65, L71, device_map, out_steps, read_steps, rst_steps, set_steps, type (+11 more)

### Community 15 - "Community 15"
Cohesion: 0.11
Nodes (19): L19, M17, M36, out_steps, read_steps, rst_steps, set_steps, type (+11 more)

### Community 16 - "Community 16"
Cohesion: 0.11
Nodes (19): L39, M18, M40, out_steps, read_steps, rst_steps, set_steps, type (+11 more)

### Community 17 - "Community 17"
Cohesion: 0.11
Nodes (19): L70, M2, M24, out_steps, read_steps, rst_steps, set_steps, type (+11 more)

### Community 18 - "Community 18"
Cohesion: 0.11
Nodes (19): M38, M52, M67, out_steps, out_steps, read_steps, rst_steps, set_steps (+11 more)

### Community 19 - "Community 19"
Cohesion: 0.12
Nodes (18): X (Digital Input) Device Map, Y (Digital Output) Device Map, Error/Alarm Handling System, Gun Coupler Sensor Interlock, HMI Button → PLC Internal Relay Mapping, Interlock System (per Line), Main Sequence Step Control (per Line), Main Sequence Flow Diagram (A3 Visual) (+10 more)

### Community 20 - "Community 20"
Cohesion: 0.15
Nodes (17): Graphify Query/Path/Explain Commands, Graphify Post-Modification Update Hook, Device/Operand Notation Rules (X/Y hex, M/D decimal, constants K/H/E), Graphify Knowledge Graph Tool, GX Works2 IL CSV Format Specification, IL Instruction Mnemonics Catalog (46 distinct, Sequence + Basic), MAIN.csv Reference Export (3607 lines), MELSEC-Q/L Programming Manual (Common Instruction) (+9 more)

### Community 21 - "Community 21"
Cohesion: 0.13
Nodes (16): 4-4. Display Mirror, code:block4 (D138 ← D160~D161  // L0 Current Vacuum (Display)), 1. Purpose, 2. Inputs, 3. Outputs, 4-1. L0 Total Usage Accumulation, 4-2. L1 Total Usage Accumulation, 4-3. Display Mirror (+8 more)

### Community 22 - "Community 22"
Cohesion: 0.18
Nodes (17): Alarm Latch System (L40~L4E — 15 Alarm Types), Conditional Jump Type Branching (CJ), Done/Fail Flags (L10~L29 — Per Step, Per Line), gmes.csv — Main Sequence Controller, Interlock Check (Safety, Vacuum, Pressure, Coupler, Supply), Per-Line Interlock Chain, Auto/Manual Mode Toggle, PC Communication (D7000/D8000 Gas Type, D7001/D8001 Target, D7012 Result, D7015 Process Code) (+9 more)

### Community 23 - "Community 23"
Cohesion: 0.12
Nodes (16): 1. Purpose, 2. Inputs, 3. Outputs, 4. Initialization Logic (First Scan Only), 5-1. X → M Mapping (Input Scan), 5-2. M → Y Mapping (Output Scan), 5-3. System Flags, 5. Main Scan Logic (Always ON, SM400) (+8 more)

### Community 24 - "Community 24"
Cohesion: 0.12
Nodes (15): 6. Lamp Control, code:block2 (// GREEN Lamp: Running + No Alarm), 1. Purpose, 2. Inputs, 3. Outputs, 6-1. L0 Interlock (L50~L55), 6-2. L1 Interlock (L60~L65), 6. Interlock 체크 (+7 more)

### Community 25 - "Community 25"
Cohesion: 0.16
Nodes (15): 1. L Device — Latch Bit (정전 유지), 2-1. System Flags (M0~M9), 2-2. Step State — Line 0 (M10~M19), 2-2b. WARMUP Flags (M520~M521), 2-2b. WARMUP Flags (M520~M521), 2-3. Step State — Line 1 (M20~M29), 2-5. Physical Input Mirrors (M300~M31F), 2-6. HMI Button Buffer (M400~M41F) (+7 more)

### Community 26 - "Community 26"
Cohesion: 0.12
Nodes (15): blocks, device_map, L16, M16, out_steps, read_steps, rst_steps, set_steps (+7 more)

### Community 27 - "Community 27"
Cohesion: 0.13
Nodes (14): 10. IL Mnemonics, 1. Purpose, 2. Inputs, 3. Outputs, 4. Alarm Detection & Latch Logic, 5. Buzzer Control, 7. Alarm Reset Logic, 8. Output Interlock (+6 more)

### Community 28 - "Community 28"
Cohesion: 0.14
Nodes (13): Emergency Stop, Exhaust Timer T3, Interlock Check, Lamp Control, blocks, meta, block_count, cpu (+5 more)

### Community 29 - "Community 29"
Cohesion: 0.13
Nodes (14): Block 10: LAMP CONTROL (Step 361–402), Block 1: MODE CONTROL (Step 0–34), Block 2: INTERLOCK CHECK (Step 35–47), Block 3: STEP L0 (Step 48–95), Block 4: STEP L1 (Step 96–143), Block 5: NG ALARM STOP (Step 144–206), Block 6: MANUAL MODE (Step 207–239), Block 7: STOP (Step 240–300) (+6 more)

### Community 30 - "Community 30"
Cohesion: 0.13
Nodes (14): Block 10: L1 EXHAUST (Step 129–138), Block 1: L0 REFRIG FAST (Step 0–25), Block 2: L0 REFRIG NORMAL (Step 26–35), Block 3: L0 OIL FAST (Step 36–51), Block 4: L0 OIL NORMAL (Step 52–59), Block 5: L0 EXHAUST (Step 60–68), Block 6: L1 REFRIG FAST (Step 69–94), Block 7: L1 REFRIG NORMAL (Step 95–104) (+6 more)

### Community 31 - "Community 31"
Cohesion: 0.18
Nodes (14): D (Data Register) Device Map, REFRIGER CHARGING MACHINE PLC Standard Program Design Report (A3), GX Works2 IL CSV Format Specification, MELSEC Q/L Programming Manual (Common Instruction), HMI Button → PLC M-Register Mapping (23 buttons), HMI Screen Navigation Flow (Operation→User Setting / Parameter / Alarm), HMI Specification for Refriger Charging Machine, Mitsubishi QCPU Q03UDV (Q mode) (+6 more)

### Community 32 - "Community 32"
Cohesion: 0.14
Nodes (13): 1. Purpose, 2. Inputs, 3. Outputs, 4. Sequence Logic (Line 0 기준, Line 1은 +10 offset), 5. Step Transition, 6. Error Conditions, 7. IL Mnemonics, code:block1 (M12 (Step Entry) ─── Rising Edge) (+5 more)

### Community 33 - "Community 33"
Cohesion: 0.14
Nodes (14): 6-1. Sequence Instructions (confirmed in MAIN.csv + MELSEC manual), 6-2. Basic Instructions (confirmed in MAIN.csv + MELSEC manual), 6-3. Notation Rules, 6. Instruction Mnemonics, 7-1. Observed device prefixes, 7-2. Notation rules, 7. Device / Operand Notation, Association Instructions (+6 more)

### Community 34 - "Community 34"
Cohesion: 0.14
Nodes (12): Block 1: GUN VACUUM (Line 0) (Step 0–35), Block 2: GUN VACUUM (Line 1) (Step 36–72), Block Detail, Block List, gunvac — IL Logic Map, Device Map, Block 1: UNIT VACUUM (Line 0) (Step 0–39), Block 2: UNIT VACUUM (Line 1) (Step 40–80) (+4 more)

### Community 35 - "Community 35"
Cohesion: 0.14
Nodes (14): L21, M0, SM400, out_steps, read_steps, rst_steps, set_steps, type (+6 more)

### Community 36 - "Community 36"
Cohesion: 0.14
Nodes (14): L1, L112, M32, out_steps, read_steps, rst_steps, set_steps, type (+6 more)

### Community 37 - "Community 37"
Cohesion: 0.15
Nodes (12): 1. Purpose, 2. 대상 센서, 3. Inputs, 4. Outputs, 5. Data Flow, 6. Communication Protocol, 7. 센서 Type 선택 아키텍처 (미래 확장), 8. Error Conditions (+4 more)

### Community 38 - "Community 38"
Cohesion: 0.15
Nodes (13): 5-1. Auto Mode Sequence, 5-2. Manual Mode, 5-3. PC Process Code Update (D7015 / D8015), 5-4. PC Result Code (D7012 / D8012), 5-5. STOP 처리, 5-6. Emergency Stop, 5. Step State Machine, code:block10 (M303 (EMG_STOP = NC OPEN) → Immediate) (+5 more)

### Community 39 - "Community 39"
Cohesion: 0.19
Nodes (12): 1. input.csv — Physical Input Mapping, 2. output.csv — Physical Output Mapping, 3. HMI Button Mapping (M → HMI), 4. Analog Input Mapping (AI → D), 5. 변경 시 영향 범위, code:block1 (// input.csv 에 정의된 대로 매 scanning 마다 X → M 복사), code:block2 (// output.csv 에 정의된 대로 매 scanning 마다 M → Y 복사), Coding Pattern (Ladder) (+4 more)

### Community 40 - "Community 40"
Cohesion: 0.22
Nodes (13): Alarm-Only Design — No Interlock, No Emergency Stop, Door Limit Sensor (좌/우, 방폭 전용), Door Open Alarm, 방폭 Explosion-Proof Type, 비방폭 Non-Explosion-Proof Type, Design Choice: Alarm Without Emergency Stop or Interlock, Door Limit Sensor Subsystem (방폭 전용), Door Open Alarm (M-relay Latch) (+5 more)

### Community 41 - "Community 41"
Cohesion: 0.15
Nodes (12): Block 1: SYSTEM FLAGS (Step 0–6), Block 2: INIT FIRST SCAN (Step 7–96), Block 3: INPUT MAPPING L0 (Step 97–129), Block 4: INPUT MAPPING L1 (Step 130–162), Block 5: OUTPUT MAPPING L0 (Step 163–189), Block 6: GLOBAL OUTPUTS (Step 190–198), Block 7: OUTPUT MAPPING L1 (Step 199–223), Block 8: CONFIG VALIDATION (Step 224–233) (+4 more)

### Community 42 - "Community 42"
Cohesion: 0.15
Nodes (13): device_map, L72, M39, out_steps, read_steps, rst_steps, set_steps, type (+5 more)

### Community 43 - "Community 43"
Cohesion: 0.15
Nodes (13): L101, M1025, out_steps, read_steps, rst_steps, set_steps, type, type (+5 more)

### Community 44 - "Community 44"
Cohesion: 0.15
Nodes (13): L114, L97, device_map, out_steps, read_steps, rst_steps, set_steps, type (+5 more)

### Community 45 - "Community 45"
Cohesion: 0.15
Nodes (13): L116, M48, out_steps, read_steps, rst_steps, set_steps, type, rst_steps (+5 more)

### Community 46 - "Community 46"
Cohesion: 0.15
Nodes (13): L79, M17, set_steps, out_steps, read_steps, rst_steps, set_steps, type (+5 more)

### Community 47 - "Community 47"
Cohesion: 0.15
Nodes (13): M21, M33, read_steps, out_steps, read_steps, rst_steps, set_steps, type (+5 more)

### Community 48 - "Community 48"
Cohesion: 0.21
Nodes (12): 485.csv RS-485 Communication, RS-485 Modbus RTU Communication Protocol, Shared D Register Output Area (485.csv + ad.csv), Auto Mode Step State Machine, gunvac.csv — Gun Vacuum Process, unitvac.csv Unit Vacuum, ΔP Vacuum Leak Detection, Vacuum Leak Test (ΔP Monitoring, Timer T2, Done L14/L24) (+4 more)

### Community 49 - "Community 49"
Cohesion: 0.21
Nodes (12): Gas Exhaust (Post-Injection, Timer T3), Injection Type 0 — 1-Sol Base (Single Solenoid), Injection Type 1 — H+L Fast+Normal (Dual Solenoid with Speed Transition), Gun Type Injection Branching (1-Sol vs H+L), refinj.csv — Refrigerant and Oil Injection, Injection Tolerance Check (±g, Over/Under Detection), Injection Tolerance Done/NG Check, Volume Calculation (HSC Pulse × Scale Factor × Correction) (+4 more)

### Community 50 - "Community 50"
Cohesion: 0.17
Nodes (11): 10. Field Content Constraints, 12. Checklist for Generating or Modifying This Format, 13. Parsing Notes (LLM-Specific), 1. File Encoding and BOM, 2. Overall File Structure (order is fixed, NO blank rows), 8. Step No. Rules, 9. END Instruction, code:block1 ([Row 1]   Program name (title)        ← see §3-1) (+3 more)

### Community 51 - "Community 51"
Cohesion: 0.17
Nodes (11): ad — IL Logic Map, Block 1: L0 PRESSURE (Step 0–6), Block 2: L0 TEMPERATURE (Step 7–15), Block 3: L0 VACUUM (Step 16–22), Block 4: L1 PRESSURE (Step 23–29), Block 5: L1 TEMPERATURE (Step 30–38), Block 6: L1 VACUUM (Step 39–45), Block 7: DISPLAY MIRROR (Step 46–59) (+3 more)

### Community 52 - "Community 52"
Cohesion: 0.21
Nodes (10): blocks, meta, block_count, cpu, generated, source_file, total_steps, test_cases (+2 more)

### Community 53 - "Community 53"
Cohesion: 0.21
Nodes (10): blocks, device_map, meta, block_count, cpu, generated, source_file, total_steps (+2 more)

### Community 54 - "Community 54"
Cohesion: 0.21
Nodes (10): blocks, device_map, meta, block_count, cpu, generated, source_file, total_steps (+2 more)

### Community 55 - "Community 55"
Cohesion: 0.20
Nodes (11): alarm.csv — Alarm Management, Buzzer and Lamp Control (M4C/M4D/M4E/M4F → Y30~Y33), EMG Priority (L40 — Immediate All RST, Highest Priority), EMG Stop Priority Processing, Output Interlock (All Sol RST on L40/L41), Output Interlock on Alarm, Emergency Stop (M303 — Immediate All Stop), Lamp Control (Green=Running, Red=Alarm, Yellow=Interlock) (+3 more)

### Community 56 - "Community 56"
Cohesion: 0.18
Nodes (11): 10-2. 봄베 알람, 11-2. 샘플링, 4-1. 시퀀스 상태 천이도 (Line 0 기준, Line 1은 M-relay +10 offset), 4-2. Gun 선택과 시퀀스 연동, 4-3. 스텝 상세 정의 (Line 0 / Line 1 공통), 4-4. 시퀀스 실패 처리, 4. 메인 시퀀스 설계 (Step Control), 5-4. VACUUM CHECK (+3 more)

### Community 57 - "Community 57"
Cohesion: 0.20
Nodes (11): 1-Line 1-Gun Machine Configuration, Operation Screen - 1 Line 1 Gun Variant, HMI Bottom Action Bar (Manual Mode, Gun Exhaust, Product Exhaust, Vacuum Check, Refriger Injection, Start, Stop), Injection Parameters Display Group (Model, Set Amount, Charging Pulse, Vacuum Pump, Actual Amount, Injection Time), Navigation Buttons (Injection Amount Setting, Operation Setting, Alarm Screen), Production Quantity Counter with Reset, Real-Time Sensor Readouts Group (Vacuum Torr, Scan Info, Pressure kgf/cm², Temperature ℃), HMI Top Control Bar (Unit Pass, Interlock, Alarm Reset, Buzzer Stop, Navigation) (+3 more)

### Community 58 - "Community 58"
Cohesion: 0.18
Nodes (10): blocks, L17, device_map, out_steps, read_steps, rst_steps, set_steps, type (+2 more)

### Community 59 - "Community 59"
Cohesion: 0.18
Nodes (10): Block 1: PC COMM L1 (Step 0–39), Block 2: PC COMM L2 (Step 40–79), Block 3: GAS TYPE MAP (Step 80–84), Block 4: AUTO BARCODE (Step 85–89), Block 5: VAC SPC DATA (Step 90–94), Block 6: RESULT CODE (Step 95–100), Block Detail, Block List (+2 more)

### Community 60 - "Community 60"
Cohesion: 0.18
Nodes (10): blocks, M32, device_map, out_steps, read_steps, rst_steps, set_steps, type (+2 more)

### Community 61 - "Community 61"
Cohesion: 0.18
Nodes (10): blocks, T7, device_map, out_steps, read_steps, rst_steps, set_steps, type (+2 more)

### Community 62 - "Community 62"
Cohesion: 0.20
Nodes (10): 4-1. Manual / Auto Toggle (M40E), 4-2. Line Select (M400 / M401), 4-3. Gun Select (M408 / M409), 4-4. PC Barcode Data 수신, 4. Mode Management, AUTO START 시 Barcode Working Area 갱신, code:block1 (M40E (HMI Momentary) → PLC Rising Edge 검출), code:block2 (M400 (LINE 0 SELECT Momentary) → Self-Hold → L70=1, L71=0) (+2 more)

### Community 63 - "Community 63"
Cohesion: 0.20
Nodes (10): Auto Mode — Full Sequence Execution, CJ Conditional Branching (Line/Gun/Oil Mode Skip), Manual Mode — Individual Function Execution, Config Validation (D270/D272 Range Check, D274 Recalc), Oil Mode (D276=1 — Oil Injection Before Refrigerant), Configuration D270/D272/D276 (Line Count, Gun Per Line, Oil Mode), Injection Model (Preset Data Table — Manual vs Barcode), Oil Mode (D276 — REF Only vs REF+OIL Injection Order) (+2 more)

### Community 64 - "Community 64"
Cohesion: 0.20
Nodes (10): 2-4. Solenoid Coil Images (M30~M6F), Global Outputs (M4C~M4F), Injection Active Flags (M60~M6F), Line 0 — Gun 0 Injection (M34~M37), Line 0 — Gun 1 Injection (M38~M3B), Line 0 — 진공 (M30~M33), Line 1 — Gun 0 Injection (M44~M47), Line 1 — Gun 1 Injection (M48~M4B) (+2 more)

### Community 65 - "Community 65"
Cohesion: 0.20
Nodes (10): 15-1. Device Address Notation, 15-2. Timer Instruction -- Use OUT not TMR, 15-3. Conditional Jump -- Avoid CJ in IL CSV, 15-4. Note Column -- Do Not Use in IL CSV, 15-5. Device Comment CSV Format, 15-6. Comparison Instruction Patterns, 15-7. Rung Size -- Avoid Ladder Too Large (C9322/C9521), 15-8. Multi-Operand Instruction Counts (Verified) (+2 more)

### Community 66 - "Community 66"
Cohesion: 0.20
Nodes (10): 3-2. D (Data Register) — Word Device (짝수 주소), Analog (Line 0 / Line 1), Calculation Work (Line 0 / Line 1), Communication, Operation Display (HMI Read) — 현재 선택 Line/Gun 기준, Parameter Settings — Line 0 (D0~D29), Parameter Settings — Line 1 (D30~D59, D228≥2), SPC (Statistics) — 전 Line 합산 (+2 more)

### Community 67 - "Community 67"
Cohesion: 0.28
Nodes (9): 485.csv — RS-485 Sensor Communication, RS-485 Sensor Polling (Modbus RTU, CRC Validation), Shared EU Output Space (D152/D156/D160/D164/D168/D172), ad.csv — Analog I/O Scaling, Analog Scaling (Raw 0~4000 → EU, 1st Order Filter), Gun Vacuum Process (Line Vacuum Sol ON, Timer T0, Done L10/L20), Unit Vacuum Process (Line Vac + Stem Sol ON, Timer T1, Done L12/L22), unitvac.csv — Unit Vacuum Process (+1 more)

### Community 68 - "Community 68"
Cohesion: 0.31
Nodes (9): 4-1. Type 0 (1-Sol Base) + D276=0 (REF Only), 4-2. Type 0 (1-Sol Base) + D276=1 (REF+OIL), 4-3. Type 1 (H+L Fast+Normal) + D276=0 (REF Only), 4-4. Type 1 (H+L) + D276=1 (REF+OIL), 4. Injection Sequences, code:block1 (VacCheck Done), code:block2 (VacCheck Done), code:block3 (VacCheck Done) (+1 more)

### Community 69 - "Community 69"
Cohesion: 0.22
Nodes (8): 1. Purpose, 2. Inputs, 3. Outputs, 5. User Settings — Gun별 (D60~D115), 7. Range Check, 8. IL Mnemonics, REFRIGER CHARGING MACHINE — Function Spec: setting.csv, 각 필드 설명

### Community 70 - "Community 70"
Cohesion: 0.33
Nodes (8): 11-1. 스케일링 (Line별), 11. 아날로그 처리 (ad.csv), 12. 데이터 플로우 요약, 13. Safety PLC 연동, 2-1. 모듈별 책임, 2. POU (Program Organization Unit) 모듈 구조, code:block3 (REF/), REFRIGER CHARGING MACHINE — PLC Program Structure Design

### Community 71 - "Community 71"
Cohesion: 0.22
Nodes (9): 0-1. 지원 구성, 0-1. 지원 구성 (Variations), 0-2. 구성 파라미터 (PLC D-Register), 0-3. Line / Gun 개념, 0-4. Gun Type, 0-5. Gun Index 공식, 0. 시스템 구성 (System Configuration), code:block1 (Line 1 ──┬── Gun A ──┐) (+1 more)

### Community 72 - "Community 72"
Cohesion: 0.25
Nodes (9): out_steps, read_steps, rst_steps, set_steps, type, D280, M0, out_steps (+1 more)

### Community 73 - "Community 73"
Cohesion: 0.25
Nodes (8): 4-1. Rule, 4-2. Worked example — Type B row `LD M1000` at step 0, 4-3. Parsing algorithm (recommended), 4-4. Generating (encoding) algorithm, 4. Universal Quoting Model (the most error-prone part), code:block4 ("0<TAB>""""<TAB>""LD""<TAB>""M1000""<TAB>""""<TAB>""""<TAB>"), code:python (def decode_record(raw: str) -> list[str]:), code:python (def encode_record(fields: list[str]) -> str:)

### Community 74 - "Community 74"
Cohesion: 0.25
Nodes (8): 5-1. Type A — Label / Line Statement row, 5-2. Type B — Instruction row (head of an instruction, with step number), 5-3. Type C — Continuation row (extra operands of the preceding instruction), 5. Three Types of Data Rows, code:block10 (Row B: [17, "", "LD=",  "K1", "", "", ""]), code:block7 ([90,   "<<<<< Q64RD Card Converting Setup Program >>>>>>>", ), code:block8 ([0,    "", "LD",   "M1000", "", "", ""]), code:block9 (Row B: [1,  "", "MOVP", "K0", "", "", ""])

### Community 75 - "Community 75"
Cohesion: 0.25
Nodes (8): 3-1. M (Internal Relay) — Bit Device (0~), 3-1. 핵심 할당 원칙, 3-2. L Device 요약 (정전유지 Bit), 3-3. M Device 요약 (Volatile Bit), 3-3. T (Timer) — 100ms 설정, 3-4. X (Digital Input) — Hex Address, 3-5. Y (Digital Output) — Hex Address, 3. 디바이스 맵 (Device Map)

### Community 76 - "Community 76"
Cohesion: 0.29
Nodes (7): reason, state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 77 - "Community 77"
Cohesion: 0.25
Nodes (7): reason, state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 78 - "Community 78"
Cohesion: 0.29
Nodes (7): reason, state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 79 - "Community 79"
Cohesion: 0.25
Nodes (7): reason, state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 80 - "Community 80"
Cohesion: 0.25
Nodes (8): out_steps, read_steps, rst_steps, set_steps, type, device_map, D340, M0

### Community 81 - "Community 81"
Cohesion: 0.71
Nodes (4): Alarm Latch Relays L64-L79, Line 0 Step Sequence M16-M24, Line 1 Step Sequence M32-M40, Vacuum Measurement Data D160 D172

### Community 82 - "Community 82"
Cohesion: 0.25
Nodes (7): alarm — IL Logic Map, Block 1: ALARM LATCH (Step 0–32), Block 2: BUZZER (Step 33–49), Block 3: ALARM RESET (Step 50–85), Block Detail, Block List, Device Map

### Community 83 - "Community 83"
Cohesion: 0.25
Nodes (8): L64, L70, rst_steps, out_steps, read_steps, rst_steps, set_steps, type

### Community 84 - "Community 84"
Cohesion: 0.25
Nodes (8): out_steps, read_steps, rst_steps, set_steps, type, D8000, M0, read_steps

### Community 85 - "Community 85"
Cohesion: 0.25
Nodes (8): M76, T0, out_steps, read_steps, rst_steps, set_steps, type, set_steps

### Community 86 - "Community 86"
Cohesion: 0.25
Nodes (8): M21, T4, out_steps, read_steps, rst_steps, set_steps, type, type

### Community 87 - "Community 87"
Cohesion: 0.25
Nodes (7): Block 1: CONFIG SYNC (Step 0–4), Block 2: GUN SETTINGS SYNC (Step 5–11), Block 3: DISPLAY MIRROR (Step 12–25), Block Detail, Block List, Device Map, setting — IL Logic Map

### Community 88 - "Community 88"
Cohesion: 0.25
Nodes (7): Block 1: L0 CYCLE DONE (Step 0–10), Block 2: L1 CYCLE DONE (Step 11–21), Block 3: DISPLAY BOMBE (Step 22–27), Block Detail, Block List, Device Map, spc — IL Logic Map

### Community 89 - "Community 89"
Cohesion: 0.25
Nodes (8): M788, T1, out_steps, read_steps, rst_steps, set_steps, type, type

### Community 90 - "Community 90"
Cohesion: 0.25
Nodes (8): L68, T2, out_steps, read_steps, rst_steps, set_steps, type, read_steps

### Community 91 - "Community 91"
Cohesion: 0.33
Nodes (7): Explosion-Proof Enclosure Type, Non-Explosion-Proof Enclosure Type, Left Door Limit Sensor, Right Door Limit Sensor, DOOR OPEN Alarm, Door Alarm Non-Latching Behavior, No Emergency Stop on Door Open

### Community 92 - "Community 92"
Cohesion: 0.29
Nodes (7): 4-1. CH0 Scaling (예: Pressure), 4-2. CH1 Scaling (예: Temperature), 4-3. CH2 Scaling (예: Vacuum), 4. Scaling Logic, code:block1 (EU = Raw × (MaxRange - MinRange) / 4000 + MinRange), code:block2 (EU = Raw × 100.0 / 4000 - 20.0), code:block3 (EU = Raw × 760.0 / 4000)

### Community 93 - "Community 93"
Cohesion: 0.38
Nodes (7): PC Barcode Data Flow (D7000/D7001 Write, D7012/D7015 Read), Barcode Model Lookup (Preset Table Comparison), setting.csv — Parameter and Configuration Management, SPC Accumulation (Total Usage, Count, Pulse per Line), Bombe Alarm (Usage ≥ Setting → L4B), spc.csv — SPC Data Accumulation, SPC Accumulator Registers (D280~D299)

### Community 94 - "Community 94"
Cohesion: 0.48
Nodes (7): idata.csv — I/O Mapping and Initialization, M→Y Output Mapping (Internal Coil to Physical Output), X→M Input Mapping (Physical Input to Internal Mirror), input.csv — X→M Mapping Definition, I/O Map — Physical X/Y to Internal M Mapping, output.csv — M→Y Mapping Definition, X→M→Y Indirect I/O Pipeline (No Direct X/Y Reference)

### Community 95 - "Community 95"
Cohesion: 0.29
Nodes (7): 11-1. Rung boundary identification, 11-2. Rung internal structure (canonical order), 11-4. Rung-boundary decision table, 11-5. Constraints summary, 11. Rung Composition Rules, code:block12 ([1] Load                  — LD / LDI / LDP / LDF            ), code:block18 (- Every rung starts with exactly one load instruction (LD / )

### Community 96 - "Community 96"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 97 - "Community 97"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 98 - "Community 98"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 99 - "Community 99"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 100 - "Community 100"
Cohesion: 0.29
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 101 - "Community 101"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 102 - "Community 102"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 103 - "Community 103"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 104 - "Community 104"
Cohesion: 0.33
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 105 - "Community 105"
Cohesion: 0.29
Nodes (6): state, updatedAt, sessionID, sources, background-task, updatedAt

### Community 106 - "Community 106"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D162, rst_steps

### Community 107 - "Community 107"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D150, type

### Community 108 - "Community 108"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D152, rst_steps

### Community 109 - "Community 109"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D154, set_steps

### Community 110 - "Community 110"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D158, out_steps

### Community 111 - "Community 111"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D160, device_map

### Community 112 - "Community 112"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D172, read_steps

### Community 113 - "Community 113"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D156, read_steps

### Community 114 - "Community 114"
Cohesion: 0.29
Nodes (7): device_map, M76, out_steps, read_steps, rst_steps, set_steps, type

### Community 115 - "Community 115"
Cohesion: 0.29
Nodes (7): L19, out_steps, read_steps, rst_steps, set_steps, type, out_steps

### Community 116 - "Community 116"
Cohesion: 0.29
Nodes (7): L21, out_steps, read_steps, rst_steps, set_steps, type, set_steps

### Community 117 - "Community 117"
Cohesion: 0.29
Nodes (7): L23, out_steps, read_steps, rst_steps, set_steps, type, type

### Community 118 - "Community 118"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D140, type

### Community 119 - "Community 119"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D148, device_map

### Community 120 - "Community 120"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D160, rst_steps

### Community 121 - "Community 121"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D7001, set_steps

### Community 122 - "Community 122"
Cohesion: 0.29
Nodes (7): device_map, M34, out_steps, read_steps, rst_steps, set_steps, type

### Community 123 - "Community 123"
Cohesion: 0.29
Nodes (7): M18, out_steps, out_steps, read_steps, rst_steps, set_steps, type

### Community 124 - "Community 124"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D172, out_steps

### Community 125 - "Community 125"
Cohesion: 0.29
Nodes (7): device_map, L32, out_steps, read_steps, rst_steps, set_steps, type

### Community 126 - "Community 126"
Cohesion: 0.29
Nodes (7): L115, out_steps, read_steps, rst_steps, set_steps, type, type

### Community 127 - "Community 127"
Cohesion: 0.29
Nodes (7): L17, out_steps, read_steps, rst_steps, set_steps, type, rst_steps

### Community 128 - "Community 128"
Cohesion: 0.29
Nodes (7): L66, out_steps, read_steps, rst_steps, set_steps, type, read_steps

### Community 129 - "Community 129"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D124, device_map

### Community 130 - "Community 130"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D62, out_steps

### Community 131 - "Community 131"
Cohesion: 0.29
Nodes (7): L69, out_steps, read_steps, rst_steps, set_steps, type, set_steps

### Community 132 - "Community 132"
Cohesion: 0.29
Nodes (7): M37, out_steps, read_steps, rst_steps, set_steps, type, read_steps

### Community 133 - "Community 133"
Cohesion: 0.29
Nodes (7): M42, out_steps, read_steps, rst_steps, set_steps, type, rst_steps

### Community 134 - "Community 134"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D128, rst_steps

### Community 135 - "Community 135"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D130, set_steps

### Community 136 - "Community 136"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D282, device_map

### Community 137 - "Community 137"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D290, type

### Community 138 - "Community 138"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D160, set_steps

### Community 139 - "Community 139"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D172, out_steps

### Community 140 - "Community 140"
Cohesion: 0.29
Nodes (7): device_map, M49, out_steps, read_steps, rst_steps, set_steps, type

### Community 141 - "Community 141"
Cohesion: 0.29
Nodes (7): L115, out_steps, read_steps, rst_steps, set_steps, type, read_steps

### Community 142 - "Community 142"
Cohesion: 0.29
Nodes (7): L19, out_steps, read_steps, rst_steps, set_steps, type, rst_steps

### Community 143 - "Community 143"
Cohesion: 0.29
Nodes (6): Block 1: VAC CHECK L0 (Step 0–21), Block 2: VAC CHECK L1 (Step 22–44), Block Detail, Block List, Device Map, vacchec — IL Logic Map

### Community 144 - "Community 144"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D160, type

### Community 145 - "Community 145"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D300, set_steps

### Community 146 - "Community 146"
Cohesion: 0.29
Nodes (7): out_steps, read_steps, rst_steps, set_steps, type, D308, rst_steps

### Community 147 - "Community 147"
Cohesion: 0.29
Nodes (7): device_map, L78, out_steps, read_steps, rst_steps, set_steps, type

### Community 148 - "Community 148"
Cohesion: 0.29
Nodes (7): L37, out_steps, read_steps, rst_steps, set_steps, type, out_steps

### Community 149 - "Community 149"
Cohesion: 0.33
Nodes (6): 11-3. Rung composition patterns, code:block13 (LD   M1000), code:block14 (LD=  K1  D4          ← (Type B: LD= K1  +  Type C: D4)), code:block15 (LD   L31), code:block16 (LD   <cond>), code:block17 (LD   SM400)

### Community 150 - "Community 150"
Cohesion: 0.33
Nodes (6): 3-1. Row 1 — Program Name (title) record, 3-2. Row 2 — PLC Information record, 3-3. Row 3 — Column header record (exactly 7 fields), 3. Detailed Rules for Each Row, code:block2 ("PLC Information:<TAB>""QCPU (Q mode) Q03UDV"""), code:block3 ("Step No.<TAB>""Line Statement""<TAB>""Instruction""<TAB>""I)

### Community 151 - "Community 151"
Cohesion: 0.33
Nodes (6): 1-1. HMI Button → PLC 내부 릴레이 (M, Momentary), 1-1. HMI Button → PLC 내부 릴레이 (M, 푸시=ON), 1-2. 물리적 I/O 추정 (2 Line 기준), 1-2. 물리적 I/O 추정 (2 Line 기준, 참고용), 1-3. Safety PLC 인터페이스, 1. I/O 요구사항 분석 (HMI → PLC 신호 매핑)

### Community 152 - "Community 152"
Cohesion: 0.33
Nodes (6): 5-1. Solenoid 동작 요약, 5-2. GUN VACUUM, 5-3. UNIT VACUUM, 5. 진공 시퀀스 상세 (gunvac.csv / unitvac.csv), code:block6 (┌──────────┐     ┌──────────┐     ┌───────────┐), code:block7 (HMI Write → M400~M416 (Momentary) → PLC Self-Hold/Toggle → L)

### Community 153 - "Community 153"
Cohesion: 0.40
Nodes (5): state, sessionID, sources, background-task, updatedAt

### Community 154 - "Community 154"
Cohesion: 0.33
Nodes (5): 485 — IL Logic Map, Block 1: RS-485 DATA MIRROR (Step 0–25), Block Detail, Block List, Device Map

### Community 155 - "Community 155"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D158

### Community 156 - "Community 156"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D160

### Community 157 - "Community 157"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D164

### Community 158 - "Community 158"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D166

### Community 159 - "Community 159"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D168

### Community 160 - "Community 160"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D170

### Community 161 - "Community 161"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D172

### Community 162 - "Community 162"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D156

### Community 163 - "Community 163"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D162

### Community 164 - "Community 164"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D164

### Community 165 - "Community 165"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D166

### Community 166 - "Community 166"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D168

### Community 167 - "Community 167"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D170

### Community 168 - "Community 168"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D300

### Community 169 - "Community 169"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D302

### Community 170 - "Community 170"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D310

### Community 171 - "Community 171"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D312

### Community 172 - "Community 172"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D314

### Community 173 - "Community 173"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D320

### Community 174 - "Community 174"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D322

### Community 175 - "Community 175"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D330

### Community 176 - "Community 176"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D332

### Community 177 - "Community 177"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D342

### Community 178 - "Community 178"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D344

### Community 179 - "Community 179"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D350

### Community 180 - "Community 180"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D352

### Community 181 - "Community 181"
Cohesion: 0.33
Nodes (6): L33, out_steps, read_steps, rst_steps, set_steps, type

### Community 182 - "Community 182"
Cohesion: 0.33
Nodes (6): L35, out_steps, read_steps, rst_steps, set_steps, type

### Community 183 - "Community 183"
Cohesion: 0.33
Nodes (6): L37, out_steps, read_steps, rst_steps, set_steps, type

### Community 184 - "Community 184"
Cohesion: 0.33
Nodes (6): L39, out_steps, read_steps, rst_steps, set_steps, type

### Community 185 - "Community 185"
Cohesion: 0.33
Nodes (6): L65, out_steps, read_steps, rst_steps, set_steps, type

### Community 186 - "Community 186"
Cohesion: 0.33
Nodes (6): L66, out_steps, read_steps, rst_steps, set_steps, type

### Community 187 - "Community 187"
Cohesion: 0.33
Nodes (6): L67, out_steps, read_steps, rst_steps, set_steps, type

### Community 188 - "Community 188"
Cohesion: 0.33
Nodes (6): L68, out_steps, read_steps, rst_steps, set_steps, type

### Community 189 - "Community 189"
Cohesion: 0.33
Nodes (6): L69, out_steps, read_steps, rst_steps, set_steps, type

### Community 190 - "Community 190"
Cohesion: 0.33
Nodes (6): L71, out_steps, read_steps, rst_steps, set_steps, type

### Community 191 - "Community 191"
Cohesion: 0.33
Nodes (6): L72, out_steps, read_steps, rst_steps, set_steps, type

### Community 192 - "Community 192"
Cohesion: 0.33
Nodes (6): L73, out_steps, read_steps, rst_steps, set_steps, type

### Community 193 - "Community 193"
Cohesion: 0.33
Nodes (6): L74, out_steps, read_steps, rst_steps, set_steps, type

### Community 194 - "Community 194"
Cohesion: 0.33
Nodes (6): L78, out_steps, read_steps, rst_steps, set_steps, type

### Community 195 - "Community 195"
Cohesion: 0.33
Nodes (6): L79, out_steps, read_steps, rst_steps, set_steps, type

### Community 196 - "Community 196"
Cohesion: 0.33
Nodes (6): M1027, out_steps, read_steps, rst_steps, set_steps, type

### Community 197 - "Community 197"
Cohesion: 0.33
Nodes (6): M1028, out_steps, read_steps, rst_steps, set_steps, type

### Community 198 - "Community 198"
Cohesion: 0.33
Nodes (6): M1872, out_steps, read_steps, rst_steps, set_steps, type

### Community 199 - "Community 199"
Cohesion: 0.33
Nodes (6): M771, out_steps, read_steps, rst_steps, set_steps, type

### Community 200 - "Community 200"
Cohesion: 0.33
Nodes (6): M775, out_steps, read_steps, rst_steps, set_steps, type

### Community 201 - "Community 201"
Cohesion: 0.33
Nodes (6): M776, out_steps, read_steps, rst_steps, set_steps, type

### Community 202 - "Community 202"
Cohesion: 0.33
Nodes (6): M777, out_steps, read_steps, rst_steps, set_steps, type

### Community 203 - "Community 203"
Cohesion: 0.33
Nodes (6): M779, out_steps, read_steps, rst_steps, set_steps, type

### Community 204 - "Community 204"
Cohesion: 0.33
Nodes (6): M791, out_steps, read_steps, rst_steps, set_steps, type

### Community 205 - "Community 205"
Cohesion: 0.33
Nodes (6): M792, out_steps, read_steps, rst_steps, set_steps, type

### Community 206 - "Community 206"
Cohesion: 0.33
Nodes (6): M793, out_steps, read_steps, rst_steps, set_steps, type

### Community 207 - "Community 207"
Cohesion: 0.33
Nodes (6): meta, block_count, cpu, generated, source_file, total_steps

### Community 208 - "Community 208"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D130

### Community 209 - "Community 209"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D142

### Community 210 - "Community 210"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D146

### Community 211 - "Community 211"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D172

### Community 212 - "Community 212"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D22

### Community 213 - "Community 213"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D50

### Community 214 - "Community 214"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D6870

### Community 215 - "Community 215"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D7000

### Community 216 - "Community 216"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D7870

### Community 217 - "Community 217"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D8001

### Community 218 - "Community 218"
Cohesion: 0.33
Nodes (6): K0, out_steps, read_steps, rst_steps, set_steps, type

### Community 219 - "Community 219"
Cohesion: 0.33
Nodes (6): K1, out_steps, read_steps, rst_steps, set_steps, type

### Community 220 - "Community 220"
Cohesion: 0.33
Nodes (6): K2, out_steps, read_steps, rst_steps, set_steps, type

### Community 221 - "Community 221"
Cohesion: 0.33
Nodes (6): K3, out_steps, read_steps, rst_steps, set_steps, type

### Community 222 - "Community 222"
Cohesion: 0.33
Nodes (6): K4, out_steps, read_steps, rst_steps, set_steps, type

### Community 223 - "Community 223"
Cohesion: 0.33
Nodes (6): L3, out_steps, read_steps, rst_steps, set_steps, type

### Community 224 - "Community 224"
Cohesion: 0.33
Nodes (6): M16, out_steps, read_steps, rst_steps, set_steps, type

### Community 225 - "Community 225"
Cohesion: 0.33
Nodes (6): M19, out_steps, read_steps, rst_steps, set_steps, type

### Community 226 - "Community 226"
Cohesion: 0.33
Nodes (6): M20, out_steps, read_steps, rst_steps, set_steps, type

### Community 227 - "Community 227"
Cohesion: 0.33
Nodes (6): M21, out_steps, read_steps, rst_steps, set_steps, type

### Community 228 - "Community 228"
Cohesion: 0.33
Nodes (6): M22, out_steps, read_steps, rst_steps, set_steps, type

### Community 229 - "Community 229"
Cohesion: 0.33
Nodes (6): M24, out_steps, read_steps, rst_steps, set_steps, type

### Community 230 - "Community 230"
Cohesion: 0.33
Nodes (6): M32, out_steps, read_steps, rst_steps, set_steps, type

### Community 231 - "Community 231"
Cohesion: 0.33
Nodes (6): M35, out_steps, read_steps, rst_steps, set_steps, type

### Community 232 - "Community 232"
Cohesion: 0.33
Nodes (6): M36, out_steps, read_steps, rst_steps, set_steps, type

### Community 233 - "Community 233"
Cohesion: 0.33
Nodes (6): M37, out_steps, read_steps, rst_steps, set_steps, type

### Community 234 - "Community 234"
Cohesion: 0.33
Nodes (6): M38, out_steps, read_steps, rst_steps, set_steps, type

### Community 235 - "Community 235"
Cohesion: 0.33
Nodes (6): M40, out_steps, read_steps, rst_steps, set_steps, type

### Community 236 - "Community 236"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D160

### Community 237 - "Community 237"
Cohesion: 0.33
Nodes (6): L114, out_steps, read_steps, rst_steps, set_steps, type

### Community 238 - "Community 238"
Cohesion: 0.33
Nodes (6): L16, out_steps, read_steps, rst_steps, set_steps, type

### Community 239 - "Community 239"
Cohesion: 0.33
Nodes (6): L33, out_steps, read_steps, rst_steps, set_steps, type

### Community 240 - "Community 240"
Cohesion: 0.33
Nodes (6): L81, out_steps, read_steps, rst_steps, set_steps, type

### Community 241 - "Community 241"
Cohesion: 0.33
Nodes (6): L97, out_steps, read_steps, rst_steps, set_steps, type

### Community 242 - "Community 242"
Cohesion: 0.33
Nodes (6): M16, out_steps, read_steps, rst_steps, set_steps, type

### Community 243 - "Community 243"
Cohesion: 0.33
Nodes (6): M18, out_steps, read_steps, rst_steps, set_steps, type

### Community 244 - "Community 244"
Cohesion: 0.33
Nodes (6): M34, out_steps, read_steps, rst_steps, set_steps, type

### Community 245 - "Community 245"
Cohesion: 0.33
Nodes (6): M49, out_steps, read_steps, rst_steps, set_steps, type

### Community 246 - "Community 246"
Cohesion: 0.33
Nodes (6): M65, out_steps, read_steps, rst_steps, set_steps, type

### Community 247 - "Community 247"
Cohesion: 0.33
Nodes (6): M772, out_steps, read_steps, rst_steps, set_steps, type

### Community 248 - "Community 248"
Cohesion: 0.33
Nodes (6): M773, out_steps, read_steps, rst_steps, set_steps, type

### Community 249 - "Community 249"
Cohesion: 0.33
Nodes (6): M788, out_steps, read_steps, rst_steps, set_steps, type

### Community 250 - "Community 250"
Cohesion: 0.33
Nodes (6): M789, out_steps, read_steps, rst_steps, set_steps, type

### Community 251 - "Community 251"
Cohesion: 0.33
Nodes (6): T7, out_steps, read_steps, rst_steps, set_steps, type

### Community 252 - "Community 252"
Cohesion: 0.33
Nodes (6): meta, block_count, cpu, generated, source_file, total_steps

### Community 253 - "Community 253"
Cohesion: 0.33
Nodes (6): L0, out_steps, read_steps, rst_steps, set_steps, type

### Community 254 - "Community 254"
Cohesion: 0.33
Nodes (6): L17, out_steps, read_steps, rst_steps, set_steps, type

### Community 255 - "Community 255"
Cohesion: 0.33
Nodes (6): L18, out_steps, read_steps, rst_steps, set_steps, type

### Community 256 - "Community 256"
Cohesion: 0.33
Nodes (6): L20, out_steps, read_steps, rst_steps, set_steps, type

### Community 257 - "Community 257"
Cohesion: 0.33
Nodes (6): L22, out_steps, read_steps, rst_steps, set_steps, type

### Community 258 - "Community 258"
Cohesion: 0.33
Nodes (6): L23, out_steps, read_steps, rst_steps, set_steps, type

### Community 259 - "Community 259"
Cohesion: 0.33
Nodes (6): L24, out_steps, read_steps, rst_steps, set_steps, type

### Community 260 - "Community 260"
Cohesion: 0.33
Nodes (6): L32, out_steps, read_steps, rst_steps, set_steps, type

### Community 261 - "Community 261"
Cohesion: 0.33
Nodes (6): L33, out_steps, read_steps, rst_steps, set_steps, type

### Community 262 - "Community 262"
Cohesion: 0.33
Nodes (6): L34, out_steps, read_steps, rst_steps, set_steps, type

### Community 263 - "Community 263"
Cohesion: 0.33
Nodes (6): L35, out_steps, read_steps, rst_steps, set_steps, type

### Community 264 - "Community 264"
Cohesion: 0.33
Nodes (6): L36, out_steps, read_steps, rst_steps, set_steps, type

### Community 265 - "Community 265"
Cohesion: 0.33
Nodes (6): L37, out_steps, read_steps, rst_steps, set_steps, type

### Community 266 - "Community 266"
Cohesion: 0.33
Nodes (6): L38, out_steps, read_steps, rst_steps, set_steps, type

### Community 267 - "Community 267"
Cohesion: 0.33
Nodes (6): L40, out_steps, read_steps, rst_steps, set_steps, type

### Community 268 - "Community 268"
Cohesion: 0.33
Nodes (6): L64, out_steps, read_steps, rst_steps, set_steps, type

### Community 269 - "Community 269"
Cohesion: 0.33
Nodes (6): L66, out_steps, read_steps, rst_steps, set_steps, type

### Community 270 - "Community 270"
Cohesion: 0.33
Nodes (6): L67, out_steps, read_steps, rst_steps, set_steps, type

### Community 271 - "Community 271"
Cohesion: 0.33
Nodes (6): L68, out_steps, read_steps, rst_steps, set_steps, type

### Community 272 - "Community 272"
Cohesion: 0.33
Nodes (6): L69, out_steps, read_steps, rst_steps, set_steps, type

### Community 273 - "Community 273"
Cohesion: 0.33
Nodes (6): L72, out_steps, read_steps, rst_steps, set_steps, type

### Community 274 - "Community 274"
Cohesion: 0.33
Nodes (6): L73, out_steps, read_steps, rst_steps, set_steps, type

### Community 275 - "Community 275"
Cohesion: 0.33
Nodes (6): M1, out_steps, read_steps, rst_steps, set_steps, type

### Community 276 - "Community 276"
Cohesion: 0.33
Nodes (6): M19, out_steps, read_steps, rst_steps, set_steps, type

### Community 277 - "Community 277"
Cohesion: 0.33
Nodes (6): M20, out_steps, read_steps, rst_steps, set_steps, type

### Community 278 - "Community 278"
Cohesion: 0.33
Nodes (6): M21, out_steps, read_steps, rst_steps, set_steps, type

### Community 279 - "Community 279"
Cohesion: 0.33
Nodes (6): M22, out_steps, read_steps, rst_steps, set_steps, type

### Community 280 - "Community 280"
Cohesion: 0.33
Nodes (6): M23, out_steps, read_steps, rst_steps, set_steps, type

### Community 281 - "Community 281"
Cohesion: 0.33
Nodes (6): M32, out_steps, read_steps, rst_steps, set_steps, type

### Community 282 - "Community 282"
Cohesion: 0.33
Nodes (6): M33, out_steps, read_steps, rst_steps, set_steps, type

### Community 283 - "Community 283"
Cohesion: 0.33
Nodes (6): M34, out_steps, read_steps, rst_steps, set_steps, type

### Community 284 - "Community 284"
Cohesion: 0.33
Nodes (6): M35, out_steps, read_steps, rst_steps, set_steps, type

### Community 285 - "Community 285"
Cohesion: 0.33
Nodes (6): M37, out_steps, read_steps, rst_steps, set_steps, type

### Community 286 - "Community 286"
Cohesion: 0.33
Nodes (6): M39, out_steps, read_steps, rst_steps, set_steps, type

### Community 287 - "Community 287"
Cohesion: 0.33
Nodes (6): M48, out_steps, read_steps, rst_steps, set_steps, type

### Community 288 - "Community 288"
Cohesion: 0.33
Nodes (6): M49, out_steps, read_steps, rst_steps, set_steps, type

### Community 289 - "Community 289"
Cohesion: 0.33
Nodes (6): M50, out_steps, read_steps, rst_steps, set_steps, type

### Community 290 - "Community 290"
Cohesion: 0.33
Nodes (6): M51, out_steps, read_steps, rst_steps, set_steps, type

### Community 291 - "Community 291"
Cohesion: 0.33
Nodes (6): M53, out_steps, read_steps, rst_steps, set_steps, type

### Community 292 - "Community 292"
Cohesion: 0.33
Nodes (6): M54, out_steps, read_steps, rst_steps, set_steps, type

### Community 293 - "Community 293"
Cohesion: 0.33
Nodes (6): M55, out_steps, read_steps, rst_steps, set_steps, type

### Community 294 - "Community 294"
Cohesion: 0.33
Nodes (6): M56, out_steps, read_steps, rst_steps, set_steps, type

### Community 295 - "Community 295"
Cohesion: 0.33
Nodes (6): M57, out_steps, read_steps, rst_steps, set_steps, type

### Community 296 - "Community 296"
Cohesion: 0.33
Nodes (6): M58, out_steps, read_steps, rst_steps, set_steps, type

### Community 297 - "Community 297"
Cohesion: 0.33
Nodes (6): M59, out_steps, read_steps, rst_steps, set_steps, type

### Community 298 - "Community 298"
Cohesion: 0.33
Nodes (6): M64, out_steps, read_steps, rst_steps, set_steps, type

### Community 299 - "Community 299"
Cohesion: 0.33
Nodes (6): M65, out_steps, read_steps, rst_steps, set_steps, type

### Community 300 - "Community 300"
Cohesion: 0.33
Nodes (6): M66, out_steps, read_steps, rst_steps, set_steps, type

### Community 301 - "Community 301"
Cohesion: 0.33
Nodes (6): M68, out_steps, read_steps, rst_steps, set_steps, type

### Community 302 - "Community 302"
Cohesion: 0.33
Nodes (6): M69, out_steps, read_steps, rst_steps, set_steps, type

### Community 303 - "Community 303"
Cohesion: 0.33
Nodes (6): M70, out_steps, read_steps, rst_steps, set_steps, type

### Community 304 - "Community 304"
Cohesion: 0.33
Nodes (6): M71, out_steps, read_steps, rst_steps, set_steps, type

### Community 305 - "Community 305"
Cohesion: 0.33
Nodes (6): M72, out_steps, read_steps, rst_steps, set_steps, type

### Community 306 - "Community 306"
Cohesion: 0.33
Nodes (6): M73, out_steps, read_steps, rst_steps, set_steps, type

### Community 307 - "Community 307"
Cohesion: 0.33
Nodes (6): M74, out_steps, read_steps, rst_steps, set_steps, type

### Community 308 - "Community 308"
Cohesion: 0.33
Nodes (6): M75, out_steps, read_steps, rst_steps, set_steps, type

### Community 309 - "Community 309"
Cohesion: 0.33
Nodes (6): M76, out_steps, read_steps, rst_steps, set_steps, type

### Community 310 - "Community 310"
Cohesion: 0.33
Nodes (6): M77, out_steps, read_steps, rst_steps, set_steps, type

### Community 311 - "Community 311"
Cohesion: 0.33
Nodes (6): M78, out_steps, read_steps, rst_steps, set_steps, type

### Community 312 - "Community 312"
Cohesion: 0.33
Nodes (6): M79, out_steps, read_steps, rst_steps, set_steps, type

### Community 313 - "Community 313"
Cohesion: 0.33
Nodes (6): M80, out_steps, read_steps, rst_steps, set_steps, type

### Community 314 - "Community 314"
Cohesion: 0.33
Nodes (6): SM401, out_steps, read_steps, rst_steps, set_steps, type

### Community 315 - "Community 315"
Cohesion: 0.33
Nodes (6): SM402, out_steps, read_steps, rst_steps, set_steps, type

### Community 316 - "Community 316"
Cohesion: 0.33
Nodes (6): L0, out_steps, read_steps, rst_steps, set_steps, type

### Community 317 - "Community 317"
Cohesion: 0.33
Nodes (6): L100, out_steps, read_steps, rst_steps, set_steps, type

### Community 318 - "Community 318"
Cohesion: 0.33
Nodes (6): L113, out_steps, read_steps, rst_steps, set_steps, type

### Community 319 - "Community 319"
Cohesion: 0.33
Nodes (6): L115, out_steps, read_steps, rst_steps, set_steps, type

### Community 320 - "Community 320"
Cohesion: 0.33
Nodes (6): L16, out_steps, read_steps, rst_steps, set_steps, type

### Community 321 - "Community 321"
Cohesion: 0.33
Nodes (6): L17, out_steps, read_steps, rst_steps, set_steps, type

### Community 322 - "Community 322"
Cohesion: 0.33
Nodes (6): L18, out_steps, read_steps, rst_steps, set_steps, type

### Community 323 - "Community 323"
Cohesion: 0.33
Nodes (6): L19, out_steps, read_steps, rst_steps, set_steps, type

### Community 324 - "Community 324"
Cohesion: 0.33
Nodes (6): L2, out_steps, read_steps, rst_steps, set_steps, type

### Community 325 - "Community 325"
Cohesion: 0.33
Nodes (6): L20, out_steps, read_steps, rst_steps, set_steps, type

### Community 326 - "Community 326"
Cohesion: 0.33
Nodes (6): L21, out_steps, read_steps, rst_steps, set_steps, type

### Community 327 - "Community 327"
Cohesion: 0.33
Nodes (6): L22, out_steps, read_steps, rst_steps, set_steps, type

### Community 328 - "Community 328"
Cohesion: 0.33
Nodes (6): L23, out_steps, read_steps, rst_steps, set_steps, type

### Community 329 - "Community 329"
Cohesion: 0.33
Nodes (6): L32, out_steps, read_steps, rst_steps, set_steps, type

### Community 330 - "Community 330"
Cohesion: 0.33
Nodes (6): L34, out_steps, read_steps, rst_steps, set_steps, type

### Community 331 - "Community 331"
Cohesion: 0.33
Nodes (6): L36, out_steps, read_steps, rst_steps, set_steps, type

### Community 332 - "Community 332"
Cohesion: 0.33
Nodes (6): L38, out_steps, read_steps, rst_steps, set_steps, type

### Community 333 - "Community 333"
Cohesion: 0.33
Nodes (6): L64, out_steps, read_steps, rst_steps, set_steps, type

### Community 334 - "Community 334"
Cohesion: 0.33
Nodes (6): L65, out_steps, read_steps, rst_steps, set_steps, type

### Community 335 - "Community 335"
Cohesion: 0.33
Nodes (6): L66, out_steps, read_steps, rst_steps, set_steps, type

### Community 336 - "Community 336"
Cohesion: 0.33
Nodes (6): L67, out_steps, read_steps, rst_steps, set_steps, type

### Community 337 - "Community 337"
Cohesion: 0.33
Nodes (6): L68, out_steps, read_steps, rst_steps, set_steps, type

### Community 338 - "Community 338"
Cohesion: 0.33
Nodes (6): L69, out_steps, read_steps, rst_steps, set_steps, type

### Community 339 - "Community 339"
Cohesion: 0.33
Nodes (6): L70, out_steps, read_steps, rst_steps, set_steps, type

### Community 340 - "Community 340"
Cohesion: 0.33
Nodes (6): L71, out_steps, read_steps, rst_steps, set_steps, type

### Community 341 - "Community 341"
Cohesion: 0.33
Nodes (6): L73, out_steps, read_steps, rst_steps, set_steps, type

### Community 342 - "Community 342"
Cohesion: 0.33
Nodes (6): L74, out_steps, read_steps, rst_steps, set_steps, type

### Community 343 - "Community 343"
Cohesion: 0.33
Nodes (6): L78, out_steps, read_steps, rst_steps, set_steps, type

### Community 344 - "Community 344"
Cohesion: 0.33
Nodes (6): L80, out_steps, read_steps, rst_steps, set_steps, type

### Community 345 - "Community 345"
Cohesion: 0.33
Nodes (6): L81, out_steps, read_steps, rst_steps, set_steps, type

### Community 346 - "Community 346"
Cohesion: 0.33
Nodes (6): L82, out_steps, read_steps, rst_steps, set_steps, type

### Community 347 - "Community 347"
Cohesion: 0.33
Nodes (6): L83, out_steps, read_steps, rst_steps, set_steps, type

### Community 348 - "Community 348"
Cohesion: 0.33
Nodes (6): L84, out_steps, read_steps, rst_steps, set_steps, type

### Community 349 - "Community 349"
Cohesion: 0.33
Nodes (6): L85, out_steps, read_steps, rst_steps, set_steps, type

### Community 350 - "Community 350"
Cohesion: 0.33
Nodes (6): L96, out_steps, read_steps, rst_steps, set_steps, type

### Community 351 - "Community 351"
Cohesion: 0.33
Nodes (6): L98, out_steps, read_steps, rst_steps, set_steps, type

### Community 352 - "Community 352"
Cohesion: 0.33
Nodes (6): L99, out_steps, read_steps, rst_steps, set_steps, type

### Community 353 - "Community 353"
Cohesion: 0.33
Nodes (6): M0, out_steps, read_steps, rst_steps, set_steps, type

### Community 354 - "Community 354"
Cohesion: 0.33
Nodes (6): M1024, out_steps, read_steps, rst_steps, set_steps, type

### Community 355 - "Community 355"
Cohesion: 0.33
Nodes (6): M1026, out_steps, read_steps, rst_steps, set_steps, type

### Community 356 - "Community 356"
Cohesion: 0.33
Nodes (6): M1032, out_steps, read_steps, rst_steps, set_steps, type

### Community 357 - "Community 357"
Cohesion: 0.33
Nodes (6): M1033, out_steps, read_steps, rst_steps, set_steps, type

### Community 358 - "Community 358"
Cohesion: 0.33
Nodes (6): M1038, out_steps, read_steps, rst_steps, set_steps, type

### Community 359 - "Community 359"
Cohesion: 0.33
Nodes (6): M1039, out_steps, read_steps, rst_steps, set_steps, type

### Community 360 - "Community 360"
Cohesion: 0.33
Nodes (6): M1040, out_steps, read_steps, rst_steps, set_steps, type

### Community 361 - "Community 361"
Cohesion: 0.33
Nodes (6): M1041, out_steps, read_steps, rst_steps, set_steps, type

### Community 362 - "Community 362"
Cohesion: 0.33
Nodes (6): M1042, out_steps, read_steps, rst_steps, set_steps, type

### Community 363 - "Community 363"
Cohesion: 0.33
Nodes (6): M1043, out_steps, read_steps, rst_steps, set_steps, type

### Community 364 - "Community 364"
Cohesion: 0.33
Nodes (6): M1044, out_steps, read_steps, rst_steps, set_steps, type

### Community 365 - "Community 365"
Cohesion: 0.33
Nodes (6): M1045, out_steps, read_steps, rst_steps, set_steps, type

### Community 366 - "Community 366"
Cohesion: 0.33
Nodes (6): M1536, out_steps, read_steps, rst_steps, set_steps, type

### Community 367 - "Community 367"
Cohesion: 0.33
Nodes (6): M1537, out_steps, read_steps, rst_steps, set_steps, type

### Community 368 - "Community 368"
Cohesion: 0.33
Nodes (6): M16, out_steps, read_steps, rst_steps, set_steps, type

### Community 369 - "Community 369"
Cohesion: 0.33
Nodes (6): M18, out_steps, read_steps, rst_steps, set_steps, type

### Community 370 - "Community 370"
Cohesion: 0.33
Nodes (6): M19, out_steps, read_steps, rst_steps, set_steps, type

### Community 371 - "Community 371"
Cohesion: 0.33
Nodes (6): M20, out_steps, read_steps, rst_steps, set_steps, type

### Community 372 - "Community 372"
Cohesion: 0.33
Nodes (6): M22, out_steps, read_steps, rst_steps, set_steps, type

### Community 373 - "Community 373"
Cohesion: 0.33
Nodes (6): M23, out_steps, read_steps, rst_steps, set_steps, type

### Community 374 - "Community 374"
Cohesion: 0.33
Nodes (6): M24, out_steps, read_steps, rst_steps, set_steps, type

### Community 375 - "Community 375"
Cohesion: 0.33
Nodes (6): M34, out_steps, read_steps, rst_steps, set_steps, type

### Community 376 - "Community 376"
Cohesion: 0.33
Nodes (6): M35, out_steps, read_steps, rst_steps, set_steps, type

### Community 377 - "Community 377"
Cohesion: 0.33
Nodes (6): M36, out_steps, read_steps, rst_steps, set_steps, type

### Community 378 - "Community 378"
Cohesion: 0.33
Nodes (6): M37, out_steps, read_steps, rst_steps, set_steps, type

### Community 379 - "Community 379"
Cohesion: 0.33
Nodes (6): M38, out_steps, read_steps, rst_steps, set_steps, type

### Community 380 - "Community 380"
Cohesion: 0.33
Nodes (6): M40, out_steps, read_steps, rst_steps, set_steps, type

### Community 381 - "Community 381"
Cohesion: 0.33
Nodes (6): M769, out_steps, read_steps, rst_steps, set_steps, type

### Community 382 - "Community 382"
Cohesion: 0.33
Nodes (6): T3, out_steps, read_steps, rst_steps, set_steps, type

### Community 383 - "Community 383"
Cohesion: 0.33
Nodes (6): L22, out_steps, read_steps, rst_steps, set_steps, type

### Community 384 - "Community 384"
Cohesion: 0.33
Nodes (6): L38, out_steps, read_steps, rst_steps, set_steps, type

### Community 385 - "Community 385"
Cohesion: 0.33
Nodes (6): M22, out_steps, read_steps, rst_steps, set_steps, type

### Community 386 - "Community 386"
Cohesion: 0.33
Nodes (6): M23, out_steps, read_steps, rst_steps, set_steps, type

### Community 387 - "Community 387"
Cohesion: 0.33
Nodes (6): M25, out_steps, read_steps, rst_steps, set_steps, type

### Community 388 - "Community 388"
Cohesion: 0.33
Nodes (6): M26, out_steps, read_steps, rst_steps, set_steps, type

### Community 389 - "Community 389"
Cohesion: 0.33
Nodes (6): M38, out_steps, read_steps, rst_steps, set_steps, type

### Community 390 - "Community 390"
Cohesion: 0.33
Nodes (6): M39, out_steps, read_steps, rst_steps, set_steps, type

### Community 391 - "Community 391"
Cohesion: 0.33
Nodes (6): M41, out_steps, read_steps, rst_steps, set_steps, type

### Community 392 - "Community 392"
Cohesion: 0.33
Nodes (6): M51, out_steps, read_steps, rst_steps, set_steps, type

### Community 393 - "Community 393"
Cohesion: 0.33
Nodes (6): M52, out_steps, read_steps, rst_steps, set_steps, type

### Community 394 - "Community 394"
Cohesion: 0.33
Nodes (6): M53, out_steps, read_steps, rst_steps, set_steps, type

### Community 395 - "Community 395"
Cohesion: 0.33
Nodes (6): M54, out_steps, read_steps, rst_steps, set_steps, type

### Community 396 - "Community 396"
Cohesion: 0.33
Nodes (6): M55, out_steps, read_steps, rst_steps, set_steps, type

### Community 397 - "Community 397"
Cohesion: 0.33
Nodes (6): M67, out_steps, read_steps, rst_steps, set_steps, type

### Community 398 - "Community 398"
Cohesion: 0.33
Nodes (6): M68, out_steps, read_steps, rst_steps, set_steps, type

### Community 399 - "Community 399"
Cohesion: 0.33
Nodes (6): M69, out_steps, read_steps, rst_steps, set_steps, type

### Community 400 - "Community 400"
Cohesion: 0.33
Nodes (6): M70, out_steps, read_steps, rst_steps, set_steps, type

### Community 401 - "Community 401"
Cohesion: 0.33
Nodes (6): M71, out_steps, read_steps, rst_steps, set_steps, type

### Community 402 - "Community 402"
Cohesion: 0.33
Nodes (6): M96, out_steps, read_steps, rst_steps, set_steps, type

### Community 403 - "Community 403"
Cohesion: 0.33
Nodes (6): M98, out_steps, read_steps, rst_steps, set_steps, type

### Community 404 - "Community 404"
Cohesion: 0.33
Nodes (6): T3, out_steps, read_steps, rst_steps, set_steps, type

### Community 405 - "Community 405"
Cohesion: 0.33
Nodes (6): T5, out_steps, read_steps, rst_steps, set_steps, type

### Community 406 - "Community 406"
Cohesion: 0.33
Nodes (6): T6, out_steps, read_steps, rst_steps, set_steps, type

### Community 407 - "Community 407"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D284

### Community 408 - "Community 408"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D292

### Community 409 - "Community 409"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D294

### Community 410 - "Community 410"
Cohesion: 0.33
Nodes (6): L24, out_steps, read_steps, rst_steps, set_steps, type

### Community 411 - "Community 411"
Cohesion: 0.33
Nodes (6): L40, out_steps, read_steps, rst_steps, set_steps, type

### Community 412 - "Community 412"
Cohesion: 0.33
Nodes (6): L75, out_steps, read_steps, rst_steps, set_steps, type

### Community 413 - "Community 413"
Cohesion: 0.33
Nodes (6): L114, out_steps, read_steps, rst_steps, set_steps, type

### Community 414 - "Community 414"
Cohesion: 0.33
Nodes (6): L18, out_steps, read_steps, rst_steps, set_steps, type

### Community 415 - "Community 415"
Cohesion: 0.33
Nodes (6): L34, out_steps, read_steps, rst_steps, set_steps, type

### Community 416 - "Community 416"
Cohesion: 0.33
Nodes (6): L35, out_steps, read_steps, rst_steps, set_steps, type

### Community 417 - "Community 417"
Cohesion: 0.33
Nodes (6): L67, out_steps, read_steps, rst_steps, set_steps, type

### Community 418 - "Community 418"
Cohesion: 0.33
Nodes (6): L81, out_steps, read_steps, rst_steps, set_steps, type

### Community 419 - "Community 419"
Cohesion: 0.33
Nodes (6): L97, out_steps, read_steps, rst_steps, set_steps, type

### Community 420 - "Community 420"
Cohesion: 0.33
Nodes (6): M16, out_steps, read_steps, rst_steps, set_steps, type

### Community 421 - "Community 421"
Cohesion: 0.33
Nodes (6): M19, out_steps, read_steps, rst_steps, set_steps, type

### Community 422 - "Community 422"
Cohesion: 0.33
Nodes (6): M32, out_steps, read_steps, rst_steps, set_steps, type

### Community 423 - "Community 423"
Cohesion: 0.33
Nodes (6): M35, out_steps, read_steps, rst_steps, set_steps, type

### Community 424 - "Community 424"
Cohesion: 0.33
Nodes (6): M50, out_steps, read_steps, rst_steps, set_steps, type

### Community 425 - "Community 425"
Cohesion: 0.33
Nodes (6): M65, out_steps, read_steps, rst_steps, set_steps, type

### Community 426 - "Community 426"
Cohesion: 0.33
Nodes (6): M66, out_steps, read_steps, rst_steps, set_steps, type

### Community 427 - "Community 427"
Cohesion: 0.33
Nodes (6): M772, out_steps, read_steps, rst_steps, set_steps, type

### Community 428 - "Community 428"
Cohesion: 0.33
Nodes (6): M773, out_steps, read_steps, rst_steps, set_steps, type

### Community 429 - "Community 429"
Cohesion: 0.33
Nodes (6): M789, out_steps, read_steps, rst_steps, set_steps, type

### Community 430 - "Community 430"
Cohesion: 0.33
Nodes (6): meta, block_count, cpu, generated, source_file, total_steps

### Community 431 - "Community 431"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D172

### Community 432 - "Community 432"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D304

### Community 433 - "Community 433"
Cohesion: 0.33
Nodes (6): out_steps, read_steps, rst_steps, set_steps, type, D306

### Community 434 - "Community 434"
Cohesion: 0.33
Nodes (6): L20, out_steps, read_steps, rst_steps, set_steps, type

### Community 435 - "Community 435"
Cohesion: 0.33
Nodes (6): L21, out_steps, read_steps, rst_steps, set_steps, type

### Community 436 - "Community 436"
Cohesion: 0.33
Nodes (6): L36, out_steps, read_steps, rst_steps, set_steps, type

### Community 437 - "Community 437"
Cohesion: 0.33
Nodes (6): L79, out_steps, read_steps, rst_steps, set_steps, type

### Community 438 - "Community 438"
Cohesion: 0.33
Nodes (6): M20, out_steps, read_steps, rst_steps, set_steps, type

### Community 439 - "Community 439"
Cohesion: 0.33
Nodes (6): M36, out_steps, read_steps, rst_steps, set_steps, type

### Community 440 - "Community 440"
Cohesion: 0.33
Nodes (6): M775, out_steps, read_steps, rst_steps, set_steps, type

### Community 441 - "Community 441"
Cohesion: 0.33
Nodes (6): M791, out_steps, read_steps, rst_steps, set_steps, type

### Community 442 - "Community 442"
Cohesion: 0.33
Nodes (6): meta, block_count, cpu, generated, source_file, total_steps

### Community 443 - "Community 443"
Cohesion: 0.40
Nodes (4): Communication, graphify, Project Conventions, Work Mode

### Community 444 - "Community 444"
Cohesion: 0.50
Nodes (5): FAST+NORMAL Simultaneous Open Injection, Gun Global Index Formula, Gun Selection-Based Injection Sequence Branching, Gun Type Definitions (Type 0-3), Gun Type per Gun Registers (D62/D76/D90/D104)

### Community 445 - "Community 445"
Cohesion: 0.40
Nodes (5): Barcode Model Lookup / Injection Amount Matching, System Configuration Parameters (D270-D276), setting.csv Parameter Management, Injection Model Preset Data Lookup, System Configuration Variations (1L1G/1L2G/2L2G/2L4G)

### Community 446 - "Community 446"
Cohesion: 0.40
Nodes (5): 3-4. D Device 요약 (Data Register), Configuration (D270~D278), Parameter — Line 0 (D0~D29), Parameter — Line 1 (D30~D59), User Settings — Gun당 14 words (D60~D115)

### Community 447 - "Community 447"
Cohesion: 0.40
Nodes (4): blocks, device_map, test_cases, warnings

### Community 448 - "Community 448"
Cohesion: 0.50
Nodes (4): 14. Official Manual Reference & List Mode Format, code:block20 ([List Mode]), Extended Instruction Set, List Mode (IL) Column Mapping

### Community 449 - "Community 449"
Cohesion: 0.50
Nodes (3): dependencies, @kilocode/plugin, @opencode-ai/plugin

### Community 450 - "Community 450"
Cohesion: 1.00
Nodes (3): Background Task State Tracking, Omo Run Continuation Session State, Sisyphus Run Continuation Session State

## Ambiguous Edges - Review These
- `Line 1 (라인1)` → `인터락 미사용 (Interlock Not Used) Button`  [AMBIGUOUS]
   · relation: safety_override
- `Line 2 (라인2)` → `인터락 미사용 (Interlock Not Used) Button`  [AMBIGUOUS]
   · relation: safety_override

## Knowledge Gaps
- **2481 isolated node(s):** `sessionID`, `state`, `updatedAt`, `sessionID`, `state` (+2476 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **11 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `Line 1 (라인1)` and `인터락 미사용 (Interlock Not Used) Button`?**
  _Edge tagged AMBIGUOUS (relation: safety_override) - confidence is low._
- **What is the exact relationship between `Line 2 (라인2)` and `인터락 미사용 (Interlock Not Used) Button`?**
  _Edge tagged AMBIGUOUS (relation: safety_override) - confidence is low._
- **Why does `device_map` connect `Community 42` to `Community 28`, `Community 36`, `Community 43`, `Community 44`, `Community 45`, `Community 46`, `Community 47`, `Community 316`, `Community 317`, `Community 318`, `Community 319`, `Community 320`, `Community 321`, `Community 322`, `Community 323`, `Community 324`, `Community 325`, `Community 326`, `Community 327`, `Community 328`, `Community 329`, `Community 330`, `Community 331`, `Community 332`, `Community 333`, `Community 334`, `Community 335`, `Community 336`, `Community 337`, `Community 338`, `Community 339`, `Community 340`, `Community 341`, `Community 342`, `Community 343`, `Community 344`, `Community 345`, `Community 346`, `Community 347`, `Community 348`, `Community 349`, `Community 350`, `Community 351`, `Community 352`, `Community 353`, `Community 354`, `Community 355`, `Community 356`, `Community 357`, `Community 358`, `Community 359`, `Community 360`, `Community 361`, `Community 362`, `Community 363`, `Community 364`, `Community 365`, `Community 366`, `Community 367`, `Community 368`, `Community 369`, `Community 370`, `Community 371`, `Community 372`, `Community 373`, `Community 374`, `Community 375`, `Community 376`, `Community 377`, `Community 378`, `Community 379`, `Community 380`, `Community 381`, `Community 382`?**
  _High betweenness centrality (0.177) - this node is a cross-community bridge._
- **Why does `Output Mapping M48-M80` connect `Community 28` to `Community 26`?**
  _High betweenness centrality (0.139) - this node is a cross-community bridge._
- **Why does `device_map` connect `Community 26` to `Community 256`, `Community 257`, `Community 258`, `Community 259`, `Community 260`, `Community 261`, `Community 262`, `Community 263`, `Community 264`, `Community 265`, `Community 266`, `Community 267`, `Community 268`, `Community 269`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 275`, `Community 276`, `Community 277`, `Community 278`, `Community 279`, `Community 280`, `Community 281`, `Community 282`, `Community 283`, `Community 284`, `Community 285`, `Community 286`, `Community 287`, `Community 288`, `Community 289`, `Community 290`, `Community 35`, `Community 291`, `Community 292`, `Community 293`, `Community 294`, `Community 295`, `Community 296`, `Community 297`, `Community 298`, `Community 299`, `Community 300`, `Community 301`, `Community 302`, `Community 303`, `Community 304`, `Community 305`, `Community 306`, `Community 307`, `Community 308`, `Community 309`, `Community 310`, `Community 311`, `Community 312`, `Community 314`, `Community 315`, `Community 313`, `Community 270`, `Community 271`, `Community 272`, `Community 273`, `Community 274`, `Community 253`, `Community 254`, `Community 255`?**
  _High betweenness centrality (0.138) - this node is a cross-community bridge._
- **What connects `sessionID`, `state`, `updatedAt` to the rest of the system?**
  _2516 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `HMI & System Configuration` be split into smaller, more focused modules?**
  _Cohesion score 0.05405405405405406 - nodes in this community are weakly interconnected._